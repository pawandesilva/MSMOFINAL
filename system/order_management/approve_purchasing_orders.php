...
// Extract data from GET and POST
extract($_GET);
extract($_POST);

// Update status on POST request
...
// QR Code generation
$qr_path = '../../qr/';
if (!file_exists($qr_path)) mkdir($qr_path);
    ...
    <div class="col-12">
        <div class="container order-container">
            ...
            <div class="invoice p-3 mb-3 border border-1 border-black" id="contentToPrint">
                <!-- Title row -->
                <div class="row" style="background-color:#91B940; color: #ffffff; padding: 10px; border-radius: 8px; height: 150px; display: flex;">
                    ...
                    <div class="col-sm-6 invoice-col mt-5">
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                <b>Status:</b> <?= $row['Status'] == '1' ? 'Approved' : '' ?><br>
                                <?php
                            }
                        }
                        ?>
                ...
                <div class="row">
                    <div class="col-12 table-responsive">
                        ...
                        $sql = "SELECT po_items.Material, po_items.Qty, po.ProductDescription, rm.MaterialName 
                                ...
                                INNER JOIN raw_materials rm ON rm.MaterialId = po_items.Material 
                                WHERE po.OrderId = '$orderid'";
                        $result = $db->query($sql);
                        ?>
                ...
                <!-- Footer -->
                <div class="row mt-3 mb-5" style="background-color:#91B940; color: #ffffff; padding: 10px; border-radius: 8px; height: 50px; display: flex;"></div>

                <!-- Actions -->
                <div class="row no-print">
                    <div class="col-12">
                        <button type="button" onclick="printContent()" class="btn btn-default"><i class="fas fa-print"></i> Print</button>
                        <button type="button" class="btn btn-success float-right"><i class="far fa-credit-card"></i> Submit Purchasing Order</button>
