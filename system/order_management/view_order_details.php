                ...
                <div class="invoice p-3 mb-3">
                    ...
                    <div class="row" style="background-color:#95D2B3; color: #ffffff; padding: 10px; border-radius: 8px;height: 75px; display: flex; border: darkgreen">
                        ...
                        <div class="col-12">
                            <?php
                            $db = dbConn();
                            $sql = "SELECT `order_date` , `FirstName`,`LastName`, `delivery_name`, `delivery_address`, `delivery_phone`, `billing_name`, `billing_address`, `billing_phone`, `order_number`, `new_order_flag`" . " FROM `orders` INNER JOIN customers ON orders.customer_id=customers.CustomerId WHERE id='$order_id' ";
                            $result = $db->query($sql);
                            ?>
                            <?php
                    ...
                    <div class="row mt-">
                        <div class="col-12 table-responsive">
                            <table class="table table-striped">
                                <?php
                                $db = dbConn();
                                $sql = "SELECT  `order_id`, `item_id`, `stock_id`, `unit_price`, `qty`,`item_name` FROM `order_items` INNER JOIN items ON order_items.item_id=items.id WHERE order_items.order_id='$order_id' ";
                                $result = $db->query($sql);
                                ?>
                                <thead>
                                    <tr>
